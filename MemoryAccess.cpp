...
void MemoryAccess::cleanUserData(const User& userId)
{
	...
	{
		//getting the picturs and untagging the removed user
		auto pictures = album->getPictures();
		for (auto picture = pictures.begin(); picture != pictures.end(); picture++)
		{
			untagUserInPicture(album->getName(), picture->getName(), userId.getId());
		}
		//if the users album then remove
...
Album MemoryAccess::openAlbum(const std::string& albumName, const int userId)
{
...
void MemoryAccess::printUsers(Pipe* pipe)
{
}
